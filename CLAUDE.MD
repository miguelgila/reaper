...
**Status:** ✅ Core functionality complete and validated (January–February 2026)
...
**Why This Works:**
1. Daemon spawns workload → daemon is the parent
...
- [x] State transitions: created → running → stopped
...
All workloads share a mount namespace with overlayfs to protect the host. Enabled by default.

```bash
# Disable overlay (run directly on host, original behavior)
