...
echo ""
echo "  ${B}# Step 1: Deploy Ansible bootstrap DaemonSet (all workers)${R}"
echo "  kubectl apply -f examples/07-ansible-complex/ansible-bootstrap-daemonset.yaml"
echo "  kubectl rollout status daemonset/ansible-bootstrap --timeout=300s"
echo ""
echo "  ${B}# Step 2: Run nginx playbook on login nodes${R}"
echo "  kubectl apply -f examples/07-ansible-complex/nginx-login-job.yaml"
